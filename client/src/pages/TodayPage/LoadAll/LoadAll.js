  ...
  render() {
    return (
      <div className="load-all">
        <button disabled={!this.state.count} onClick={this.handleClick}>
          Загрузить на весь день
        </button>
        {this.state.count ? (
          <p className="events-count">
            {this.state.count} мероприятий ≈ {(this.state.count * 1.3).toFixed(2)} кб
          </p>
        ) : (
          <p className="events-count">Мероприятий нет</p>
        )}
